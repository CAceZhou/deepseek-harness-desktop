...
# 用法：powershell -File scripts/fetch-runtime.ps1 [-NodeVersion 24.19.0] [-DshVersion 0.1.0-rc.8]
[CmdletBinding()]
param(
...
# 4. 精简运行时（删除文档/测试/类型声明/非 win32-x64 二进制等，约省 100+MB）
& (Join-Path $PSScriptRoot 'prune-runtime.ps1') -RuntimeDir $dest

# 5. 冒烟：真实拉起 web 服务并验证 200 响应（含前端 dist 是否随包发布）
$smokePort = 39871
...
# 6. pnpm standalone（壳内置：dsh plugin 的 spawnSync("pnpm") 经 pnpm.cmd 解析到它；
...
Set-Content (Join-Path $dest 'RUNTIME_VERSIONS.txt') "node $NodeVersion`r`ndsh $DshVersion`r`npnpm $PnpmVersion"
