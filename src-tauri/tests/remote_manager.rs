...
async fn start_with_port_in_use_errors() {
    // 占住回环端口（allow_lan=false 时代理绑 127.0.0.1:同端口必然失败；
    // 不能用 0.0.0.0——Windows 允许回环地址与通配符并存，不会撞上）
    let blocker = std::net::TcpListener::bind(("127.0.0.1", 0)).unwrap();
    let port = blocker.local_addr().unwrap().port();
    let (mgr, _ctx, _statuses) = make_manager(port, ssh_off(), false, false, false, None);
...
async fn full_chain_up_then_off() {
    ...
    let (mgr, _ctx, _statuses) = make_manager(remote_port, ssh_off(), false, true, false, Some(dsh_port));
    let s = mgr.start().await;
...
async fn ssh_tunnel_up_uses_server_url() {
    ...
    let (mgr, _ctx, _statuses) =
        make_manager(remote_port, ssh_on("vps.example.com", 8080, 0), false, false, true, Some(dsh_port));
    mgr.start().await;
...
async fn port_change_applies_on_next_start() {
    ...
    let (mgr, ctx, _statuses) = make_manager(port_a, ssh_off(), false, true, false, Some(dsh_port));
    let s = mgr.start().await;
    assert_eq!(s.phase, "up");
...
async fn cloudflare_tunnel_up_uses_cf_url() {
    ...
    let remote_port = free_port().unwrap();
    let (mgr, _ctx, _statuses) = make_manager(remote_port, ssh_off(), true, false, false, Some(dsh_port));
    let s = mgr.start().await;
...
async fn cloudflare_drives_when_coexisting_with_ssh() {
    ...
    let remote_port = free_port().unwrap();
    let (mgr, _ctx, _statuses) =
        make_manager(remote_port, ssh_on("vps.example.com", 8080, 0), true, false, false, Some(dsh_port));
    let s = mgr.start().await;
    assert_eq!(s.phase, "starting");
    let deadline = Instant::now() + Duration::from_secs(30);
    loop {
...
/// 默认局域网访问未开启（allow_lan=false）：局域网直连 start 直接转 error，
/// 提示在设置里开启「允许局域网访问」；即使 allow_http 开启也不放行局域网
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn lan_access_disabled_by_default_errors() {
    let dsh_port = free_port().unwrap();
    let remote_port = free_port().unwrap();
    let (mgr, _ctx, _statuses) = make_manager(remote_port, ssh_off(), false, false, true, Some(dsh_port));
    let s = mgr.start().await;
    assert_eq!(s.phase, "error", "局域网访问未开启时应报错");
    let err = s.error.unwrap();
    assert!(
        err.contains("允许局域网访问"),
        "错误应提示开启局域网开关：{err}"
    );
    assert!(s.link.is_none());
}

/// 局域网开关与 allow_http 解耦：仅开 allow_lan（allow_http 仍关）时局域网直连
/// 可用、生成 http:// 链接（局域网链路天然明文 HTTP，允许局域网即放行其链路）
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn lan_access_works_without_plaintext_http_switch() {
    let dsh_port = free_port().unwrap();
    let work = tempfile::tempdir().unwrap();
    let mut dsh = spawn_fixture_dsh(dsh_port, work.path());
    wait_dsh_ready(dsh_port).await;
    ...
    let link = s.link.unwrap();
    assert!(
        link.starts_with("http://") && link.contains(&format!(":{remote_port}/?token=")),
        "仅开局域网开关即可生成 http 局域网链接：{link}"
    );
    // 局域网链路 cookie 应为非 Secure（明文 http 可用）
    let token = link.rsplit("?token=").next().unwrap().to_string();
    let http = reqwest::Client::builder()
        .redirect(reqwest::redirect::Policy::none())
        .no_proxy()
    ...
    let r = http
        ...
        .unwrap();
    assert_eq!(r.status(), 302);
    let set_cookie = r.headers().get("set-cookie").unwrap().to_str().unwrap().to_string();
    assert!(!set_cookie.contains("Secure"), "局域网链路 cookie 不应带 Secure：{set_cookie}");
...
async fn ssh_link_https_when_http_disallowed() {
    ...
    let remote_port = free_port().unwrap();
    let (mgr, _ctx, _statuses) =
        make_manager(remote_port, ssh_on("vps.example.com", 8080, 0), false, false, false, Some(dsh_port));
    mgr.start().await;
    let deadline = Instant::now() + Duration::from_secs(20);
    loop {
        let s = mgr.status();
