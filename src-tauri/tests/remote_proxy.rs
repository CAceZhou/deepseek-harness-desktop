...
async fn gate_requires_token() {
    ...
    loop {
        if let Ok(r) = reqwest::get(format!("http://127.0.0.1:{dsh_port}/")).await {
    ...
    let t0 = Instant::now();
    let r = http.get(format!("{base}/?token=wrong")).send().await.unwrap();
    assert_eq!(r.status(), 403, "错误 token 应 403");
    assert!(t0.elapsed() >= Duration::from_millis(400), "错误 token 应有延迟");
