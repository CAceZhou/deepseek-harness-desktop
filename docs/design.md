...
**Tests**: `tests/remote_proxy.rs` (gate 403/302/cookie incl. no-Secure assertion/forward/WS bridge/503/shutdown releases the port/plugin-bundle ternary rewrite), `tests/remote_manager.rs` (busy port → error; fixture dsh + fixed port full chain up→stop; config change applies on next start; reset rotates token, port unchanged; SSH tunnel up → link uses the server address; SSH failure via a fake-ssh marker → error with the ssh diagnostic surfaced), plus a `ssh_tunnel.rs` unit test pinning the command-line shape (BatchMode/ExitOnForwardFailure/-R 0.0.0.0:port:…). Real-device LAN and real SSH-server paths are not automated; manual acceptance: LAN → start from tray → phone on the same Wi-Fi scans the QR; SSH → fill in the server info → start from tray → remote browser opens http://server:expose-port.
