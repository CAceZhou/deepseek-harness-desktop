...
│  remote/     remote access: LAN + SSH reverse tunnel + Cloudflare tunnel │
│              supervision                                                   │
...
Result: staging 344 to ~280 MB (including the bundled 52 MB cloudflared); installer **58.8 MB**; installed ~294 MB (the bulk is node.exe at ~90 MB plus dsh's dependency tree and node-pty's native terminal module; nothing left is safe to cut).
...
| --- | --- | --- |
| Rust unit tests (167) | runtime deploy/fallback/path normalization/copy-progress callback, progress stage weights & percent mapping, theme BOM parsing & first-launch seeding, notify frame classification/subagent book/summary, LogRing eviction, port allocation & readiness, platform basics, zoom clamp/persistence/hook embeds settings, settings model/validation/persistence/sound enum/notification-rule gating & legacy-key migration/remote-port normalization/SSH-tunnel config validation/allow_http default-off & roundtrip, skills frontmatter parsing/list/toggle/delete/import conflicts, mcp patch parsing/toggle/delete/upsert validation & advanced-key preservation/seed marker/three-source parsing & import conflicts, remote tunnel-URL parsing & token redaction/link composition/SSH host normalization & link scheme (allow_http ⇒ forced https)/LAN-IP probing, ssh_tunnel command-line shape | `cd src-tauri && cargo test` |
...
## 16. Remote access (LAN fixed port + SSH tunnel + Cloudflare Quick Tunnel + embedded auth proxy)

One click on the tray "Remote access" item gives a phone/remote browser the **full dsh Web UI** via a token-bearing link. Three outward modes coexist: **LAN direct** (`0.0.0.0:<fixed port>`), **SSH reverse tunnel** (to your own public server), and **Cloudflare Quick Tunnel** (`cloudflared.exe` bundled in the runtime; anonymous ephemeral tunnel, outbound-only — no public IP, port mapping, or firewall pinhole). HTTPS-only by default (Cloudflare / your server's TLS reverse proxy); **plaintext HTTP is an independent opt-in (`allow_http`, off by default — see `remote/http.rs`)**.

**Chain**:

```
a) LAN: phone browser —HTTP→ PC 0.0.0.0:<fixed port> remote::proxy (token gate)
     → 127.0.0.1:<dsh port> dsh web (HTTP + /api/events.* WS)
b) public (own server): browser —HTTPS→ server:<expose port> (TLS reverse proxy)
     → SSH -R reverse tunnel → PC 127.0.0.1:<fixed port> → token gate → dsh
c) public (zero server): browser —HTTPS→ Cloudflare edge → cloudflared (desktop, outbound-only)
     → 127.0.0.1:<fixed port> remote::proxy (token gate) → dsh
```

**Modules** (`src-tauri/src/remote/`):

- `mod.rs` — `RemoteManager`: lifecycle (start/stop/status), token generation (fresh 256-bit hex on every start), 6 invoke commands (start_remote/stop_remote/get_remote_status/copy_remote_link/get_remote_qr/reset_remote_link). Runtime config (fixed port + SSH tunnel + Cloudflare + allow_http) is read from a `RemoteConfig` watch channel, so settings changes apply without a restart. The auth proxy binds `0.0.0.0:<fixed port>` (LAN direct). The surfaced link follows one dominant mode: **Cloudflare > SSH > LAN IP** (cf_drives drives phase/link; the other tunnel runs in the background and only logs). With HTTP disallowed (allow_http=false, default), LAN-only start errors out and SSH links are forced to https. Tunnel events compose `link = {url}/?token={token}`; status changes broadcast `remote-status`, update tray submenu enabled states, and append to events.log. `reset_link` rotates the token in place and drops all live sessions (the revocation path for a leaked link); only available while Starting/Up, address and port unchanged.
- `proxy.rs` — axum reverse proxy bound to `0.0.0.0:<fixed port>` (directly reachable on the LAN). Auth: a valid `__dsh_remote` cookie forwards directly; a matching `?token=` (constant-time compare) → 302 that strips the token + sets an HttpOnly cookie (**`Secure` is decided by `remote/http.rs`'s `allow_http`: off by default ⇒ Secure — browsers never store/send Secure cookies over plain http, so the auth chain dies there, i.e. HTTP is unsupported by default; enabling drops `Secure`**); a mismatched token → fixed 500ms delay then 403; no credentials → 403 gate page. **Plain HTTP additionally gets a secure-context polyfill (`remote/http.rs`)**: without it `crypto.randomUUID()`/`navigator.clipboard` are undefined (non-secure context) and the dsh UI crashes; injected at the `<head>` start via `find_head_open_end` only when `allow_http` is on (not needed under HTTPS). HTTP forwards via reqwest streaming (3xx passed through, never followed); WS upgrades are terminated at the proxy then re-established to dsh with frame-level bidirectional bridging. The dsh port is read from a `watch::Receiver`, so dsh restarts don't break the proxy. **The token lives in a shared cell (RwLock) and the gate reads the latest value per request** — after a reset the old link/cookie die instantly; **WS bridges also select on a drain Notify** (`enable()` registers interest up front so a reset during connect can't slip through), and reset/shutdown `notify_waiters()` drops every established connection — otherwise an attacker's already-open page keeps receiving the event stream after revocation. **Browser-marker headers must be stripped when forwarding** (`origin`/`referer`/`sec-fetch-*`): dsh's /api trust fence (dsh-client-connection `isTrustedApiRequest`) requires Origin.host == Host header and rejects `sec-fetch-site: cross-site`; over LAN/tunnel the Origin is `http://<lan-ip>:<port>` or the trycloudflare domain, so every RPC call from the page would 403. Stripped, the request looks like an Origin-less loopback client to dsh (the WS bridge side already sends no Origin in the tungstenite handshake, so it passes natively). **The forwarding client must use `.no_proxy()`** — otherwise a user-level system proxy (e.g. Clash) hijacks the 127.0.0.1 forwarding. **Plugin-bundle rewrite**: `/plugins/*/client.js` responses are buffered (≤4MB, identity encoding only) and every `connection.isLoopback ? "host" : "memory"` is replaced with `"host"` — dsh's internal-testing notice (WelcomeNoticeStore) picks memory persistence for non-loopback origins, so through the tunnel domain the acknowledgement never reaches settings.yaml and the modal pops on every visit; rewritten, remote clients share the host-persisted acknowledgement with the desktop (which has already confirmed, so remote never pops). The rewrite path strips `accept-encoding` (forcing identity) and conditional request headers (no 304) from the forwarded request, and drops `content-length`/`etag` from the response; if the needle is absent (dsh changed the wording upstream) the body passes through untouched — the notice still pops, but nothing breaks.
...
- `ssh_tunnel.rs` — SSH reverse-tunnel supervision via the system OpenSSH client: `ssh -N -T -o BatchMode=yes -o ExitOnForwardFailure=yes -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 -o ServerAliveInterval=30 -o ServerAliveCountMax=3 -p <ssh port> -i <key> -R 0.0.0.0:<expose port>:127.0.0.1:<fixed port> user@server`. Error keywords (Permission denied / remote port forwarding failed, …) trigger backoff restarts; after 5 failures it lands in a terminal Failed state that surfaces the error; 8s alive without error → Up; stop via kill tree + Job fallback. The server needs `GatewayPorts yes` for public reachability.
- `http.rs` — the standalone plaintext-HTTP support with its switch (off by default): `SECURE_CONTEXT_POLYFILL` (crypto.randomUUID/navigator.clipboard patch for non-secure contexts), `cookie_attributes` (off ⇒ Secure; on ⇒ no Secure), `link_scheme` (off ⇒ https always; on ⇒ follows the server-address prefix).

**Security model**: the link is the credential (tray/QR page warn "do not share"); the token regenerates on every start, and stopping remote access or quitting the app invalidates it immediately (quit order: stop remote first, then dsh). On a leak, "Reset link" revokes with one click (token rotation + dropping live sessions, address and port unchanged). The auth proxy listens on `0.0.0.0:<fixed port>`; dsh itself still binds loopback only. **The token never touches logs**: events.log records only phase/url/error/proxy_port (never the link), any `?token=` query string is redacted via `redact_token`, and the up-toast body omits the link (system notification center keeps history) — it just points to the tray menu. Known trade-off: **plaintext HTTP (no TLS) is an opt-in feature (`allow_http`) that defaults off** — by default remote access is HTTPS-only (Cloudflare tunnel / your server's TLS reverse proxy). When enabled, LAN direct access and non-TLS SSH expose ports are plaintext and sniffable along the path — mitigated by "trusted-network assumption + token rotated every start + one-click revocation on leak"; use it on networks you trust. Links are not stable either — phones cannot bookmark them (each start requires a fresh scan), in exchange for the shortest possible leak window.
...
**UI**: tray submenu (start/stop mutually exclusive enabled, copy link, show QR, reset remote link — copy/QR/reset only available when Up) + the `#/remote` local window (QR SVG generated by the `qrcode` crate, copy button, reset button with confirm, toggle) + a "remote access" status row in the diagnostics panel + settings toggles/cards for "Allow plaintext HTTP access" (off by default) plus SSH tunnel and Cloudflare config. After a locale-switch tray-menu rebuild, the `TrayRemoteItems` handles are replaced and enabled states re-applied from the current phase.

**Tests**: `tests/remote_proxy.rs` (gate 403/302/cookie incl. allow_http-off ⇒ Secure / on ⇒ no-Secure, and no randomUUID polyfill when HTTP is disallowed/forward/WS bridge/503/shutdown releases the port/plugin-bundle ternary rewrite), `tests/remote_tunnel.rs` (fake-cloudflared.cjs: URL parsing, crash restart, stop), `tests/remote_manager.rs` (busy port → error; fixture dsh + fixed-port full chain up→stop; config change applies next start; reset rotates token, port unchanged; SSH tunnel up → link uses the server address; **HTTP off by default: LAN-only start errors, SSH link forced to https**; SSH failure surfaces the ssh diagnostic; missing cloudflared → error; Cloudflare coexisting with SSH). The real-tunnel path is not automated (needs outbound internet and a real server); manual acceptance: LAN → enable "Allow plaintext HTTP access" in settings → start from tray → phone on the same Wi-Fi scans the QR; SSH → fill in the server info → start from tray → remote browser opens the server link (https by default; plaintext HTTP requires the setting).
