...
- **Centered on first launch**: the main window (tauri.conf `"center": true`) and the five on-demand tray windows (builder `.center()`) all default to screen center; the window-state plugin's restore is queued at window_created and runs before the first visible frame, so a remembered geometry overrides the centered default — first launch centered, later launches at the previous position, with no visible flash of the default (verify-no-size-flash.ps1 asserts the first visible frame already has the remembered geometry).
...
- NSIS hooks (`src-tauri/windows/nsis-hooks.nsh`, wired via `bundle.windows.nsis.installerHooks`, **path relative to `src-tauri`**): `NSIS_HOOK_PREINSTALL`/`NSIS_HOOK_PREUNINSTALL` first `taskkill /F /T /IM DSHDesktop.exe` to kill the whole tree, then use PowerShell to sweep any remaining process whose executable lives under `$INSTDIR` (orphaned node.exe/cloudflared.exe left by ≤0.1.8), then sleep 1.5s so the kernel can release file handles. The template's built-in `CheckIfAppIsRunning` kills only the main binary and cannot reach child processes — relying on it alone reproduces the "Can't write" failure.
