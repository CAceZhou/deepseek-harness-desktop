...
**测试**：`tests/remote_proxy.rs`（门岗 403/302/种 cookie（含 allow_http 关→带 Secure、开→无 Secure 断言，以及 HTTP 关闭时不注入 randomUUID polyfill）/转发/WS 桥接/503/shutdown 释放端口/插件 bundle 三元式改写/HTML 文档移动端样式与信息标签页脚本注入、无 </head> 透传）、`tests/remote_manager.rs`（端口占用报错、fixture dsh + 固定端口全链路 up→stop、改配置下次生效、重置轮换 token 端口不变、SSH 隧道 up 后链接用服务器地址、**默认禁止 HTTP：局域网直连报错 / SSH 链接强制 https**、SSH 失败（fake-ssh 打 Permission denied）转 error 透出错误）、`remote/ssh_tunnel.rs` 单元测试锚定命令行形态（BatchMode/ExitOnForwardFailure/-R 0.0.0.0:port:…）。局域网真机与真实 SSH 服务器链路不进自动化，手动验收：局域网 → 设置开启"允许明文 HTTP 访问" → 托盘开启 → 手机连同一 Wi-Fi 扫码；SSH → 在设置填好服务器信息 → 托盘开启 → 异地浏览器打开服务器地址链接（默认 https，明文 HTTP 需在设置开启）。
